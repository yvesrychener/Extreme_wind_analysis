...
print(se_df_jan)
```
The deviance and standard deviations suggest taking r=2 is a reasonable choice


Plot the diagnostics for r=2 (May, good example):
```{r}
#---- Change i to view different months (for loop unfortunately not possible)
i=5
#----
...
Fit the r-largest-order statistic
```{r}
largest_2 = get_monthly(apply.monthly(prod_ts, function(x) x[order(x, decreasing=T)[1:2]]))
...
** LOOK AT PARAMETER SEAONALITY WITH DIFFERENT METHODS **
```{r}
...
**PART 2**
We perform 2 tests to check dependence, first for linear depenence, then for step dependence (trend = int(renormalise(trend)>0))
...
Now the same for step dependance (ENSO)
```{r}
ratios = list()
# split nino data into months
n = nino34
...
Now the same for step dependance (time)
```{r}
...
```
The fit in february has completely failed, and the others are ok.
...
Now, we calculate the return levels using the bayesian approach:
...
It seems that the confidence intervals for chi and chibar contain the 0 horizontal line, we can therefore not conclude that cape and srh are dependant.



**Part 7**
We will now fit different bivariate models to the monthly maxima and compare them using AIC. We will also estimate the dependance using the negative logistic link. (if the confidence interval for dep does not contain 0, then we can conclude that there is dependance).
...
for (i in 1:12) {
  ...
  # uncomment this and change fit to be plotted
  #par(mfrow=c(3,2)) 
  #plot(fit1)
...
We believe that there is no reason to assume that a different model should be used for different months, so we chose the negative logistic model. It is one of the best models in every month, and if it isn't, the difference is only very small.
...
for (i in 1:12) {
  ...
  # USE QUANTILES TO GET RETURN LEVELS
