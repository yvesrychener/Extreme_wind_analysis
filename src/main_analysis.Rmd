...
par(mfrow=c(1,1))
plot_w_err(1:12, dep.estimate, dep.sd, "Monthly Dependance Parameter vs Independance (Red)")
...
for (i in 1:12) {
  ...
  params = bv_fits[[i]][[mod_ix]]$param
  
  print(params)
  # Pass appropriate parameters according to model
  if (mod_ix %in% c("log","neglog")) {
  ...
  else if (mod_ix %in% c("bilog", "ct", "negbilog")) {
    sim_vals = rbvevd(n=n_sims,
  ...
  print(head(sim_prod))
  bivar_100yr_retlvl[i]=nth(sim_prod, n_sims/100)
  bivar_50yr_retlvl[i]=nth(sim_prod, n_sims/50)
